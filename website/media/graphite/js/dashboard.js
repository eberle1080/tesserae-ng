...
function showHelp() {
  var win = new Ext.Window({
    ...
    width: 550,
    height: 300,
    autoLoad: "/graphite/dashboard/help/"
  });
  win.show();
}
  ...
  */
  ...
  if (existingIndex > -1) {
    ...
    var record = new GraphRecord({
      ...
      params: myParams,
      url: '/graphite/render?' + Ext.urlEncode(urlParams)
    });
...
function updateGraphRecords() {
  graphStore.each(function (item, index) {
    ...
    item.set('url', '/graphite/render?' + Ext.urlEncode(params));
...
function refreshGraph(index) {
  ...
  record.set('url', '/graphite/render?' + Ext.urlEncode(record.get('params')));
...
function doShare() {
  if (dashboardName == null) {
    Ext.Ajax.request({
      url: "/graphite/dashboard/create-temporary/",
      method: 'POST',
      params: {
...
function sendSaveRequest(name) {
  Ext.Ajax.request({
    url: "/graphite/dashboard/save/" + name,
    method: 'POST',
    params: {
      state: Ext.encode( getState() )
    },
...
function sendLoadRequest(name) {
  Ext.Ajax.request({
    url: "/graphite/dashboard/load/" + name,
    success: function (response) {
               var result = Ext.decode(response.responseText);
               if (result.error) {
...
function deleteDashboard(name) {
  Ext.Ajax.request({
    url: "/graphite/dashboard/delete/" + name,
    success: function (response) {
      var result = Ext.decode(response.responseText);
      if (result.error) {

...
from website.graphite.util import getProfile, getProfileByUsername
from website.graphite.logger import log
from website.graphite.account.models import Profile, MyGraph, Variable, View, Window
...
def _draw(request,targets,_from=None,until=None,template=None,window=None,interval=None):
    ...
    url = '/graphite/render?' + urllib.urlencode(params)
    if window:
      w = window
      out += "win = %s_win;\n" % w
      out += "img_id = '%s_img';\n" % w
